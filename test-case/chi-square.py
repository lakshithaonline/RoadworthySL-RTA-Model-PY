...
# Example test.joblib data
# Each row is a sample with all features
# Example test.joblib data with all required features
test_data = pd.DataFrame({
    ...
    'Final Score': [70, 60, 80, 50, 90]  # Add this feature if it was used during training
})

# Example true labels for test.joblib data
# Replace these with your actual labels
true_labels = [1, 0, 1, 0, 1]  # 1 for pass, 0 for fail

# Predict outcomes with both models
predictions_lr = model_lr.predict(test_data)
predictions_rf = model_rf.predict(test_data)

contingency_table = pd.crosstab(predictions_lr, predictions_rf, rownames=['Logistic Regression'], colnames=['Random Forest'])
